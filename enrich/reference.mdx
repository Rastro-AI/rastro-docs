...
| `status` | string | `"running"`, `"pending_review"`, `"completed"`, or `"failed"` |
...
| `pending_review` | All results ready, staged for review | Yes — all results |
| `completed` | Job finished successfully | Yes — all results |
| `failed` | Job failed or was cancelled | Yes — partial results from completed batches |

<Note>
Cancelled jobs are reported as `failed` with `stopped_early: true` in `service_usage`. Partial results and credits for completed items are preserved.
</Note>
