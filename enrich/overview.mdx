...
| `staged_status` | string | No | `"pending"` | Status for staged changes: `"pending"` (visible to admin) or `"draft"` (vendor draft) |
...
| `staged_change_id` | string | ID of the staged change record |
...
<Tip>
See [GET /public/enrich/{'{job_id}'}](/enrich/get-job) for full polling endpoint documentation.
</Tip>
