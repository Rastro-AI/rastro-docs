...
# Now enrich using just the catalog_id - no need to repeat output_schema
response = requests.post(
    "https://catalogapi.rastro.ai/api/public/enrich",
    headers={"Authorization": "Bearer YOUR_API_KEY"},
    json={
...
The catalog's `schema_definition` fields are automatically used as the `output_schema`. Any taxonomy or quality settings configured on the catalog are also applied.
